...
public class PlayerController : MonoBehaviour {
	...
	private bool isGrounded = true;//判断玩家是否着落
	[HideInInspector]
	...
	void Update () {
		...
		if (!die) {		//stop doing anything when player dead - 
			if (Input.GetKeyDown (KeyCode.W)) {		//Only jump when player on the ground
				Jump();
			}
	...
	void FixedUpdate(){
		if (!die) {		//stop doing anything when player dead
            ...
            //    Debug.Log("overlapCircle isGrounded is " + isGrounded);
	...
	public void Jump(){
		if (!die) {		//stop doing anything when player dead
			...
			if (isUsingJetPack) {	//if using jet pack
                ...
                float f = 100f;	//inscrease fx effect

                JetPackFire = GetComponent<ParticleSystem>();
                var emission = JetPackFire.emission;
	...
	public void JumpOff(){
		isJumpHold = false;
		if (isUsingJetPack) {
            float f = 25f;	//inscrease fx effect
